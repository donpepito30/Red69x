...
import { MOCK_MODELS } from '@/lib/mockModelsData';
  ...
  const fetchLiveModels = useCallback(async (isSilent = false) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.client_models && data.client_models.length > 0) {
          setModels(data.client_models);
          setLastRefreshed(new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' }));
          return;
        }
      }
      // Fallback to local MOCK_MODELS if API response is empty or not ok
      setModels(MOCK_MODELS);
      setLastRefreshed(new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' }));
    } catch (e) {
      console.warn('Network or API issue, using local mock catalog:', e);
      setModels(MOCK_MODELS);
      setLastRefreshed(new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' }));
    } finally {
